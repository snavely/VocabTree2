 ...
 * of keypoints. */
int ReadKeyFile(const char *filename, short int **keys, 
                keypt_t **info = NULL);
...
int ReadKeysGzip(gzFile fp, short int **keys, keypt_t **info = NULL);
...
ANNkd_tree *CreateSearchTree(int num_keys, short int *keys);

/* Compute likely matches between two sets of keypoints */
std::vector<KeypointMatch> MatchKeys(int num_keys1, short int *k1, 
				     int num_keys2, short int *k2,
				     double ratio = 0.6, 
...
std::vector<KeypointMatch> MatchKeysExhaustive(int num_keys1, short int *k1, 
                                               int num_keys2, short int *k2,
                                               double ratio);
